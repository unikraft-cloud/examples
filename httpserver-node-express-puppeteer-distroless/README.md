# Distroless Puppeteer HTTP Server
...
2. Clone the [`examples` repository](https://github.com/unikraft-cloud/examples) and `cd` into the `examples/httpserver-node-express-puppeteer-distroless/` directory:

   ```bash
   git clone https://github.com/unikraft-cloud/examples
   cd examples/httpserver-node-express-puppeteer-distroless/
   ```
...
**Using the unikraft CLI (Recommended)**
```bash title="unikraft"
unikraft build . --output <my-org>/httpserver-node-express-puppeteer-distroless:latest
unikraft run --metro fra \
  -m 4G \
  -p 443:3000/tls+http \
  --scale-to-zero policy=idle,cooldown-time=1000,stateful=true \
  --image <my-org>/httpserver-node-express-puppeteer-distroless:latest
```
...
```ansi title="unikraft"
metro:        fra
name:         httpserver-node-express-puppeteer-distroless-7afg3
uuid:         7bb479d7-5b3e-444f-b07c-eae4da6f57cc
state:        starting
image:        <my-org>/httpserver-node-express-puppeteer-distroless
resources:
  memory:     4096MiB
  vcpus:      1
service:
...
[●] Deployed successfully!
 ...
 ├──────── image: oci://unikraft.io/<my-org>/httpserver-node-express-puppeteer-distroless@sha256:78d0b180161c876f17d05116b93011ddcd44c76758d6fa0359f05938e67cea65
 ├─────── memory: 4096 MiB
 ├────── service: little-snow-7qwu6vv5
 ├─ private fqdn: httpserver-node-express-puppeteer-distroless-7afg3.internal
 └─── private ip: 10.0.3.1
```

In this case, the instance name is `httpserver-node-express-puppeteer-distroless-7afg3`.
...
fra    httpserver-node-express-puppeteer-distroless-7afg3  running  <my-org>/httpserver-node-express-puppeteer-distroless        4096MiB  1      nameless-fog-0tvh1uov.fra.unikraft.app  2 minutes ago
```
...
httpserver-node-express-puppeteer-distroless-7afg3  nameless-fog-0tvh1uov.fra.unikraft.app  running  since 6mins  oci://unikraft.io/<my-org>/httpserver-node-express-puppeteer-distroless@s...  4.0 GiB  1            15.27 ms
```
...
**Using the unikraft CLI (Recommended)**
```bash title="unikraft"
unikraft instances delete httpserver-node-express-puppeteer-distroless-7afg3
```
...
**Using the legacy kraft CLI**
```bash title="kraft"
kraft cloud instance remove httpserver-node-express-puppeteer-distroless-7afg3
```
