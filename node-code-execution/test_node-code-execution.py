"""End-to-end test for the ``node-code-execution`` example.

Mirrors the manual steps from ``node-code-execution/README.md``:

1. ``unikraft build . --output <prefix>/node-code-exec:<tag>``
2. ``unikraft run --metro <metro> --name <tpl> -m 512M --image ...``
   (the instance auto-converts into a template)
3. ``unikraft build rom1/ --output <prefix>/node-rom1:<tag>``
   ``unikraft build rom2/ --output <prefix>/node-rom2:<tag>``
4. ``unikraft run --metro <metro> -p 443:8080/tls+http
   --rom image=<prefix>/node-rom1:<tag>,at=/rom --template <tpl>``
