...
def test_tyk_hello(build_image, run_instance, http, wait_instance, test_run_id):
    # Internal domain for the Redis instance. Unique per test run so parallel
    # CI runs (e.g. matrix legs sharing a metro) don't collide on the same
    # domain; set TYK_REDIS_DOMAIN to override.
    redis_domain = os.environ.get("TYK_REDIS_DOMAIN") or f"tyk-redis-{test_run_id}.internal"
    ...
    # TYK_GW_STORAGE_HOST tells Tyk which Redis host to use; tyk.conf leaves
    # storage.host empty, so this variable is required.
